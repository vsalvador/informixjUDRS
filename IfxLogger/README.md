...
Informix Post Commit Trigger allows to publish database changes as JSON messages to a file. 
...
This script copies main jar file "IfxLogger.jar" to $INFORMIXDIR/extend/krakatoa.
...
EXECUTE PROCEDURE sqlj.install_jar('file:$INFORMIXDIR/extend/krakatoa/IfxLogger.jar', 'IfxLogger', 1);
```
...
```sql
execute procedure j_log('{"operation":"insert", "table":"state", "owner":"informix", "database":"stores_demo", "txnid":12124695617764, "commit_time":1621529490, "rowdata":{"code":"53", "sname":"ES"}}');
execute procedure j_log('{"operation":"update", "table":"state", "owner":"informix", "database":"stores_demo", "txnid":12124695625976, "commit_time":1621529490, "rowdata":{"code":"53", "sname":"UK"}, "before_rowdata":{"code":"53", "sname":"ES"}}');
execute procedure j_log('{"operation":"delete", "table":"state", "owner":"informix", "database":"stores_demo", "txnid":12124695638244, "commit_time":1621529490, "rowdata":{"code":"53", "sname":"UK"}}');
```
...
IfxLogger.jar library is configured to automaticaly register the j_log procedure in the database when required. If this autopublish method doesn't works, you can register the procedure manualy by executing this SQL statements in your database:

```sql
DROP PROCEDURE IF EXISTS j_log;
CREATE PROCEDURE j_log(lvarchar)
  external name 'IfxLogger.log(java.lang.String)'
  language java;

grant execute on procedure j_log(lvarchar) to public;
```
